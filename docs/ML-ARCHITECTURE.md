...
模型，并每 30 天更迭一次；标签清除区间和模型可用时间戳保持独立。V4 将监督目标
改为与账本完全一致的 `确认线收盘决策 -> 下一根开盘成交 -> 12 根后开盘退出`，
标签跨度为 13 根，并保留 1 根 embargo。模型固定为低复杂度、固定种子的
...
当 25% 目标仓位超过 5 分钟报价成交量容量时，V4 缩小到容量且记录 clip，不把整笔
订单伪装为可成交。

2026-08-21 的 canonical V4 开发回放覆盖 840 天、28 代和 172 笔闭环交易：24 bps
