...
def test_remote_storage_requires_delete_journal_and_full_sync(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    path = tmp_path / "state.sqlite3"
    monkeypatch.setattr(runtime, "is_remote_storage", lambda _path: True)
    with sqlite3.connect(path) as db:
        assert configure_sqlite_connection(db, path) == "delete"
        assert str(db.execute("PRAGMA journal_mode").fetchone()[0]).lower() == "delete"
